...
def one_million_obfuscated(classifier):
    ...
    X2 = MD.load_user_item_matrix_1m_masked(file_index= 117)
...
def lastFM(classifier):
    ...
    random_state = np.random.RandomState(0)
    # model = Models.Dominant_Class_Classifier()
    model = LogisticRegression(penalty='l2', C=0.1, random_state=random_state) # penalty='l2', C=545.5594781168514, random_state=random_state) #
    from sklearn.svm import SVC
    # model = SVC(kernel='linear', probability=True, random_state=random_state)
    # from sklearn.dummy import DummyClassifier
    # model = DummyClassifier(strategy='most_frequent')
...
def flixster_obfuscated(classifier):
    ...
    LFM_obfuscated (Classifiers.svm_classifier) #log_reg svm_classifier
